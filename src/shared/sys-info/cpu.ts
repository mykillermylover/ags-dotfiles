...
export const cpu = Variable(cpuBuffer)
  .poll(sysInfo.pollInterval, (prev) => {
    prevBuffer = prev;
...
export const getCpuPercentage
  = cpu(current => getCorePercentage(prevBuffer, current));
export const getCoresPercentage
  = cpu(current => getCpuCoresPercentage(prevBuffer, current));

const getCpuCoresPercentage
  = (prevInfo: GTop.glibtop_cpu, currentInfo: GTop.glibtop_cpu) => {
...
const getCorePercentage
  = (prev: CpuCoreInfo, current: CpuCoreInfo) => {
